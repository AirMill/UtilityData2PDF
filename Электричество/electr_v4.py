...
def create_widgets(frame, row, value1, value2, value3, value4, saved_values):
    label_text = f"{
        value1} - {value2} - {value4}" if value1 is not None else ''
    label = ttk.Label(frame, text=label_text, font='Arial 11 bold')
    label.grid(row=row, column=0, padx=5, pady=5, sticky='w')

    entry_var = tk.StringVar(value=saved_values.get(
        f"{value1}_{value2}_{value4}", ""))
    entry_vars.append(entry_var)

    # Check if the entry is preloaded from JSON
...
def on_entry_click(event, entry, is_preloaded):
    # Check if the entry is preloaded from JSON
    if is_preloaded:
        # Change the font to Arial 11 bold when clicked
        entry.config(font='Arial 11 bold', background='white')
        # Clear the italic styling
        entry.icursor(0)
...
def update_gui():
    ...
    data = load_data('res/счетчики_электричество.xlsx')
    ...
    for i, (value1, value2, value3, value4) in enumerate(data):
        create_widgets(frame_middle, i, value1, value2,
                       value3, value4, saved_values1)
...
def send_email():
    ...
    sender_email = 'pov1286@mail.ru'
    receiver_email = 'air.mill@gmail.com'
    subject = f"Subject: Показания электричества - {current_datetime}"
    body = 'Переданные показания по электричеству'
    ...
    server = smtplib.SMTP('smtp.mail.ru', port=465)
    server.starttls()
    # Replace with your email password
    server.login(sender_email, 'eq8JrswJxSzhb1brAfQp')
...
def generate_pdf():
    current_datetime = datetime.now().strftime("%d%m%Y")
    global pdf_filename
    pdf_filename = f"Показания_электроснабжение_{current_datetime}.pdf"
    ...
    pdf_canvas_obj.drawCentredString(
        300, 750, f'Показания электроснабжения за: {datetime.now().strftime("%B %Y")}')

    pdf_canvas_obj.setFont("ArialUnicode", 15)
    pdf_canvas_obj.drawCentredString(
        300, 810, f"Протокол показаний счетчиков электроснабжения. ")
    pdf_canvas_obj.drawCentredString(
...
# Add this function to update the global pdf_filename


def update_pdf_filename():
    global pdf_filename
    current_datetime = datetime.now().strftime("%d%m%Y")
    pdf_filename = f"Показания_электроснабжение_{current_datetime}.pdf"
...
window = tk.Tk()
window.title('-=Электроснабжение=-')
window.geometry('800x700+500+100')
...
label1 = tk.Label(frame_top1,
                  text=f'Показания - Электроснабжение - {
...
submit_button.pack(ipady=20, ipadx=100)
